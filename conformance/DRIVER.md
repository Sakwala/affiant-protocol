...
|---|---|---|
| **Inference** | `given.gate.inference` | Reports exactly the scripted fields for every turn, unchanged — no invention, no filtering, no re-scoring. Absent or `null`: reports nothing. The driver must also put **`given.ctx.utterance` in front of the inference step**: from `v0.1.3` PV-3 has the implementation establish presence from that text and verify the port's `presence` and `utteranceSpan` against it. The driver passes that text, and `""` where the fixture states none — an empty utterance, in which nothing hits and no claim verifies. A driver that passes *nothing at all* is a different thing: it puts the implementation on PV-3's no-utterance path, where the port's report stands unverified, and it is not a conforming driver. |
