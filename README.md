...
- 2026-09-08 — **`v0.1.3`: PV-3 says who establishes presence — the implementation, from the utterance.** The rule's parenthetical read "the value is literally present in the utterance; with an `utterance-span` binding when the inference port supplies offsets", which left the grade in the inference port's hands. No shipped port asks a model for `presence` at all, so both implementations fell through to `Inferred` and every value a person typed in chat was sworn **"AI suggested"** on the Evidence Card ([`Sakwala/affiant#123`](https://github.com/Sakwala/affiant/issues/123)). PV-3 now states the condition itself: the implementation finds the value text in the unmodified turn — ordinal, case-insensitive, with neighbouring characters that are absent or are neither letters nor digits — grades `Conversation` on a hit and `Inferred` otherwise, and binds a hit to `{ offset, length, hash }` over the **utterance's own** substring. The port's `presence` and `utteranceSpan` become hints verified the same way: a `literal` the text does not confirm is `Inferred`, and a value the text does carry is `Conversation` even where the port said nothing. A caller with no utterance in hand keeps the port's report, and no port is asked to change.

  [`conformance/RUNNER.md`](conformance/RUNNER.md) and [`conformance/fixture.schema.json`](conformance/fixture.schema.json) make both hints optional and say what they are; [`conformance/lint/lint.mjs`](conformance/lint/lint.mjs) runs the finder over every fixture's own utterance and refuses one that scripts a hint the text contradicts without pinning the grade the finder gives. Two fixtures arrive with the amendment — [`gate/inference-presence-computed-from-the-utterance`](conformance/fixtures/gate/18-inference-presence-computed-from-the-utterance.json) and [`gate/inference-port-literal-unconfirmed`](conformance/fixtures/gate/19-inference-port-literal-unconfirmed.json) — **authored here rather than promoted**, because neither implementation had the behaviour to copy; they are the amendment's negative oracle against `1.0.0-beta.3` ([`conformance/ORACLE.md`](conformance/ORACLE.md)). No schema, no wire, no vector and no existing fixture changed: every scripted `literal` already hits its own utterance and every scripted `inferred` already misses.

  The suite is **65 documents** from here. Both published runs read at `v0.1.2`, where it was 63, and both are expected to fail the two new fixtures until each implementation lands the change; the parity manifests move when they re-pin, and the tag and the "current tag" lines move with the release commit.
